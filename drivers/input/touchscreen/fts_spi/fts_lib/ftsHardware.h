...
/*CHUNKS*/
#define READ_CHUNK                                                             \
	1024 /*chunk dimension of a single i2c read, max allowed value is 2kB*/
#define WRITE_CHUNK                                                            \
	1024 /*chunk dimension of a single i2c write, max allowed value is 2kB*/
#define MEMORY_CHUNK                                                           \
	1024 /*chunk dimenasion of a single i2c write on mem, max allowed value is 2kB*/
...
#define IER_DISABLE                                                            \
	0x00 /*value to write in IER_ADDR to disable the interrupts*/
...
#define FLASH_PANEL_PAGE_START                                                 \
	26 /*starting page which contain Panel Init data*/
...
#define FLASH_ADDR_CODE                                                        \
	0x00000000 /*starting address (words) in the flash of the code in FTI*/
#define FLASH_ADDR_CONFIG                                                      \
	0x00007C00 /*starting address (words) in the flash of the config in FTI*/
#define FLASH_ADDR_CX                                                          \
	0x00007000 /*starting address (words) in the flash of the Init data in FTI*/
...
#define FIFO_DEPTH                                                             \
	32 /*max number of events that the FIFO can collect before going in overflow in FTM5*/
...
#define DUMMY_FRAMEBUFFER                                                      \
	1 /*1 if the first byte read from Frame buffer is dummy*/
...
#define DUMMY_FRAMEBUFFER                                                      \
	0 /*1 if the first byte read from Frame buffer is dummy*/
...
#define ADDR_FRAMEBUFFER                                                       \
	((u64)0x0000000000000000) /*frame buffer address in memory*/
#define ADDR_ERROR_DUMP                                                        \
	((u64)0x000000000000EF80) /*start address dump error log*/
...
#define ADDR_BOOT_OPTION                                                       \
	((u64)0x0000000020000025) /*address of Boot option register in SONIA*/

/*INTERRUPT INFO*/
#define ADDR_IER                                                               \
	((u64)0x0000000020000029) /*address of the Interrupt enable register in FTMI*/

/*Chip ID/Fw Version*/
#define ADDR_DCHIP_ID ((u64)0x0000000020000000) /*chip id address for FTI*/
#define ADDR_DCHIP_FW_VER                                                      \
	((u64)0x0000000020000004) /*fw version address for FTI*/

/*INTERFACE REGISTER*/
#define ADDR_ICR                                                               \
	((u64)0x000000002000002D) /*address of Device control register to set the comunication protocol (SPI/I2C)*/
#define SPI4_MASK 0x02 /*bit to set spi4*/

/*CRC ADDR*/
#define ADDR_CRC                                                               \
	((u64)0x0000000020000078) /*address of the CRC control register in FTI*/
#define CRC_MASK                                                               \
	0x03 /*bitmask which reveal if there is a CRC error in the flash*/

#define ADDR_CONFIG_OFFSET                                                     \
	((u64)0x0000000000000000) /*config address in memory if FTI*/
